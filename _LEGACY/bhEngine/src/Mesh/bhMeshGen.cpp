...
{
  _Allocate(4, 6, 1);
  prims[0].baseVertex = 0;  prims[0].numVerts = 4;
  prims[0].baseIndex = 0;  prims[0].numInds = 6;

  _sizex *= 0.5f;
  _sizey *= 0.5f;

  if (config.HasBinding(MESH_BINDING_POSITIONS))
  {
    // xy plane, normal +z
  ...
  if (config.HasBinding(MESH_BINDING_NORMALS))
  {
    verts[0].normal = verts[1].normal = verts[2].normal = verts[3].normal = { 0.0f, 0.0f, 1.0f };
  }
  if (config.HasBinding(MESH_BINDING_UV_0))
  {
    for (short i = 0; i < 4; ++i)
    {
  ...
  if (config.HasBinding(MESH_BINDING_TANGENTS))
  {
...
{
  _Allocate(24, 36, 1);
  prims[0].baseVertex = 0;  prims[0].numVerts = 24;
  prims[0].baseIndex = 0;  prims[0].numInds = 36;
...
{
  _Allocate(16, 24, 1);
  prims[0].baseVertex = 0;  prims[0].numVerts = 16;
  prims[0].baseIndex = 0;  prims[0].numInds = 24;
