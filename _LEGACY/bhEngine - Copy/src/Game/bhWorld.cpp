...
static bhWorldMaterial* s_worldMat{ nullptr };
static bhWorldPipeline* s_worldPipeline{ nullptr };
...
bool bhWorld::Init()
{
  bhGPUContext* gpuCtx = bhSystem::GPUContext();
  s_worldPipeline = gpuCtx->CreateWorldPipeline();

  bhMesh* meshWall = bhSystem::MeshCache()->GetMesh("Wall_01");
  meshWall->Setup().AddBinding(MESH_BINDING_POSITIONS).AddBinding(MESH_BINDING_NORMALS).AddBinding(MESH_BINDING_UV_0).AddBinding(MESH_BINDING_TANGENTS);
  meshWall->CreatePlane(glm::vec2(blockSize.side, blockSize.side));
  gpuCtx->UploadWorldMesh(s_worldPipeline, meshWall);

  bhMesh* meshSuzanne = bhSystem::MeshCache()->GetMesh("Suzanne.bms");
  gpuCtx->UploadWorldMesh(s_worldPipeline, meshSuzanne);
  DEBUG_testMesh = meshSuzanne;

  s_worldMat = gpuCtx->LoadWorldMaterial("Wall_01.xml");

  camera.SetProjection(bhCamera::DEFAULT_FOV_Y_DEGS, bhSystem::WindowASpect(), 0.1f, 1000.0f);
...
void bhWorld::Destroy()
{
  ...
  gpuCtx->DestroyWorldMaterial(s_worldMat);
  //DestroyResources();
}
...
void bhWorld::Render()
{
  bhGPUContext* rd = bhSystem::GPUContext();
  rd->BindWorldPipeline(s_worldPipeline);
  rd->SetWorldCameraView(s_worldPipeline, &camera.GetViewData());
  rd->BindWorldMaterial(s_worldMat);
  rd->DrawWorldMesh(s_worldPipeline, DEBUG_testMesh, xForm);
      ...
      for (int lvl = 0; lvl < 3; ++lvl)
      {
        ...
        for (int m = 0; m < BH_NUM_GRID_DIRECTIONS; ++m)
        {
          ...
          if (wm)
          {
            ...
            rd->DrawWorldMesh(s_worldPipeline, wm, pos);
          }
        }
...
bool bhWorld::UpdateBlock(int x, int y, int z)
{
  ...
  {
    bhMesh* msh = bhSystem::MeshCache()->GetMesh("Wall_01");
    block.wallMeshes[BH_GRID_EAST] = msh;
  }
  else
  {
  ...
  {
    bhMesh* msh = bhSystem::MeshCache()->GetMesh("Wall_01");
    block.wallMeshes[BH_GRID_NORTH] = msh;
  }
  else
  {
  ...
  {
    bhMesh* msh = bhSystem::MeshCache()->GetMesh("Wall_01");
    block.wallMeshes[BH_GRID_WEST] = msh;
  }
  else
  {
  ...
  {
    bhMesh* msh = bhSystem::MeshCache()->GetMesh("Wall_01");
    block.wallMeshes[BH_GRID_SOUTH] = msh;
  }
  else
  {
  ...
  {
    bhMesh* msh = bhSystem::MeshCache()->GetMesh("Wall_01");
    block.wallMeshes[BH_GRID_BOTTOM] = msh;
  }
  else
  {
  ...
  {
    bhMesh* msh = bhSystem::MeshCache()->GetMesh("Wall_01");
    block.wallMeshes[BH_GRID_TOP] = msh;
  }
  else
  {
